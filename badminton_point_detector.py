#!/usr/bin/env python3
"""
Enhanced Badminton Point Detection System
Detects when a point is being played vs when players are idling
with improved temporal smoothing to handle edge cases
"""
...
class EnhancedBadmintonPointDetector:
    def __init__(self, movement_threshold=0.005, min_point_duration=30, 
        ...
        """
        Initialize the enhanced point detector
        ...
        Args:
            ...
            idle_grace_period: Frames to wait before switching to idle (prevents false idle during brief pauses)
            point_grace_period: Frames to wait before switching to point (prevents false points during brief movements)
        """
        ...
        self.last_movement_values = deque(maxlen=5)  # Track recent movement for context
    ...
    def handle_state_transition(self, old_state, new_state):
        ...
        if new_state == 'point_playing' and old_state == 'idle':
            ...
            # Point ended
            print(f"Point #{self.point_count} ended")
    
    def process_video(self, video_path, output_path=None, results_path='enhanced_point_detection_results.csv'):
    ...
    def draw_visualization(self, frame, state, movement, frame_number):
        ...
        cv2.rectangle(frame, (10, 10), (350, 100), bg_color, -1)
...
def main():
    parser = argparse.ArgumentParser(description='Enhanced Badminton Point Detection System')
    parser.add_argument('--input', '-i', required=True, help='Input video file')
    parser.add_argument('--output', '-o', help='Output video file')
    parser.add_argument('--results', '-r', default='enhanced_point_detection_results.csv', 
    ...
    detector = EnhancedBadmintonPointDetector(
    ...
    print("Starting enhanced badminton point detection...")
